...
describe('simile mark (%)', () => {
  it('resolves % to the previous bar cells', () => {
    const row = parseRow('| Am | % |');
    expect(row.bars[0].cells).toMatchObject([chord('A', 'minor')]);
    expect(row.bars[1].cells).toMatchObject([chord('A', 'minor')]);
  });

  it('resolves multiple consecutive % bars', () => {
    const row = parseRow('| C | % | % | % |');
    for (const bar of row.bars) {
      expect(bar.cells).toMatchObject([chord('C')]);
    }
  });

  it('resolves % to the most recent non-simile bar', () => {
    const row = parseRow('| G | D | % |');
    expect(row.bars[0].cells).toMatchObject([chord('G')]);
    expect(row.bars[1].cells).toMatchObject([chord('D')]);
    expect(row.bars[2].cells).toMatchObject([chord('D')]);
  });
  ...
  it('resolves % when last real bar has multiple slots', () => {
    ...
    expect(row.bars[0].cells).toMatchObject(expected);
    expect(row.bars[1].cells).toMatchObject(expected);
  });
  ...
  it('% with a non-single closing barline preserves that barline', () => {
    ...
    expect(row.bars[1].cells).toMatchObject([chord('C')]);
  });
  ...
  it('parses a full song with simile marks', () => {
    ...
    expect(rows[0].bars[2].cells).toMatchObject([chord('D')]);
    expect(rows[1].bars[1].cells).toMatchObject([chord('C')]);
    expect(rows[1].bars[2].cells).toMatchObject([chord('C')]);
  });
