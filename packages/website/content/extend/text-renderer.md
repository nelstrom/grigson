...
layout: base.njk
title: Text Renderer
permalink: /extend/text-renderer/
---
...
The text renderer supports all the same configuration options as the SVG renderer, except for `layout` (which has no meaning for text output). See the [SVG renderer](/extend/svg-renderer/) for the full configuration reference.
...
See the [CLI Reference](/tooling/cli/) for details.
